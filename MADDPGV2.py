...
def get_update(current_key, binary_sequence):
    kkk = np.zeros(len(current_key))
    for i in range(len(current_key)):
        if current_key[i]!=binary_sequence[i]:
            kkk[i]=1
    return kkk
...
def run():
    ...
    #node_attribute_all = np.reshape(node_attribute_all, (11, 9, 10))
    attribute_length = 10
    key_length = 5
    num_agent = 2
    reward_all = []
    ...
    step_length = 7
    alice = Agent(name='Alice', num_obs=attribute_length+key_length, num_act=key_length, num_agent=num_agent, num_node=num_node)
    bob = Agent(name='Bob', num_obs=attribute_length+key_length, num_act=key_length, num_agent=num_agent, num_node=num_node)
    # Discount factor for future rewards
    gamma = 0.99
    # Used to update target networks
    start_flag = False
    std_dev = 1
    memory = Buffer(buffer_capacity=memory_size, batch_size=batch_size, num_agent=num_agent, num_obs=attribute_length+key_length, num_node=num_node, num_act=key_length)
    alice_idx = 1
    bob_idx = 4
    for epoch in range(training_epoch):
        ...
        for step in range(step_length):
            ...
            pen = 0
            if (alice_current_key==a_binary_sequence).all() != True and (bob_current_key==b_binary_sequence).all() != True:
                pen = 1
            ...
            alice_current_key = get_update(alice_current_key, a_binary_sequence)
            bob_current_key = get_update(bob_current_key, b_binary_sequence)
