...
class Agent:
    def __init__(self, name, num_obs, num_act, num_agent, action_bound):
        ...
        def get_actor():
            ...
            outputs = layers.Dense(num_act * (num_agent - 1), activation="tanh", kernel_initializer=last_init)(out)
    ...
    def save_model(self, epoch):
        self.actor.save(self.name+'_actor'+'_'+str(epoch))
        self.critic.save(self.name+'_critic'+'_'+str(epoch))
...
class AgentRestore:
    def __init__(self, name, num_obs, num_act, num_agent, epoch):
        ...
        def restore_actor():
            model = keras.models.load_model(self.name+'_actor'+'_'+str(self.epoch))
            return model

        def restore_critic():
            model = keras.models.load_model(self.name + '_critic'+'_'+str(self.epoch))
            return model
