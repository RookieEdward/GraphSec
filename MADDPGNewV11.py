...
def run():
    ...
    bob = Agent(name='Calvin', num_obs=attribute_length+2, num_act=key_length, num_agent=num_agent, num_node=num_node)
    ...
    for epoch in range(training_epoch):
        ...
        for step in range(step_length):
            ...
            if memory.buffer_counter >= batch_size*30:
                ...
                with tf.GradientTape() as tape:
                    y = bt_reward + gamma * alice.critic_target([bt_state_, bt_bob_act_, bt_alice_act_], training=True)
                    critic_value = alice.critic([bt_state, bt_bob_act, bt_alice_act], training=True)
                    critic_loss = tf.math.reduce_mean(tf.math.square(y - critic_value))
                critic_grad = tape.gradient(critic_loss, alice.critic.trainable_variables)
                alice.critic_optimizer.apply_gradients(zip(critic_grad, alice.critic.trainable_variables))
