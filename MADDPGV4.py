...
def run():
    ...
    for epoch in range(training_epoch):
        ...
        for step in range(step_length):
            ...
            obs_alice = np.concatenate((alice_attribute, alice_current_key[step,:]), axis=0)
            obs_bob = np.concatenate((bob_attribute, bob_current_key[step,:]), axis=0)
            ...
            obs_alice_ = np.concatenate((alice_attribute_, a_binary_sequence), axis=0)
            obs_bob_ = np.concatenate((bob_attribute_, b_binary_sequence), axis=0)
